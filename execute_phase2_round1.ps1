...
try {
    Write-Host "=== Phase-2 Round-1 Joint Tuning ===" -ForegroundColor Cyan

    $entryStrategies = @(
        Get-Content strategy_evaluation/phase2_round1_entry_strategies.txt |
        Where-Object { $_ -and $_.Trim().Length -gt 0 }
    )
    $exitStrategies = @(
        Get-Content strategy_evaluation/phase2_round1_exit_strategies.txt |
        Where-Object { $_ -and $_.Trim().Length -gt 0 }
    )
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Phase-2 Round-1 Joint Tuning failed"
    }

    $duration = (Get-Date) - $startTime
    Write-Host ""
    Write-Host "=== Phase-2 Round-1 Joint Tuning completed ===" -ForegroundColor Green

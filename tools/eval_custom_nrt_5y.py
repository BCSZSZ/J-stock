...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate custom N/R/T grid across 5 years.")
    parser.add_argument("--n-values", default="12,16", help="Comma-separated N values")
    parser.add_argument("--r-values", default="4.0,4.8", help="Comma-separated R values")
    parser.add_argument("--t-values", default="2.0,2.2", help="Comma-separated T values")
    ...
    parser.add_argument("--tag", default="nrt_grid", help="Output filename tag")
    ...
    d = args.d
    b = args.b
    tag = args.tag.strip() or "nrt_grid"
    ...
    summary = (
        df.groupby(["exit_strategy", "N", "R", "T"], as_index=False)
        .agg(
    ...
    hold_summary = (
        tdf.groupby(["exit_strategy", "N", "R", "T"], as_index=False)
        .agg(avg_hold=("holding_days", "mean"))
        .merge(
            win_trades.groupby(["exit_strategy", "N", "R", "T"], as_index=False)
            .agg(avg_win_ret=("return_pct", "mean"), avg_win_hold=("holding_days", "mean")),
            on=["exit_strategy", "N", "R", "T"],
            how="left",
        )
        .merge(
            loss_trades.groupby(["exit_strategy", "N", "R", "T"], as_index=False)
    ...
    trigger = (
        tdf.groupby(["exit_strategy", "N", "R", "T", "exit_urgency"], as_index=False)
        .size()
        .sort_values(["exit_strategy", "size"], ascending=[True, False])
    )
    ...
    raw_path = out_dir / f"custom_{tag}_raw_{ts}.csv"
    summary_path = out_dir / f"custom_{tag}_summary_{ts}.csv"
    hold_path = out_dir / f"custom_{tag}_hold_{ts}.csv"
    trade_path = out_dir / f"custom_{tag}_trades_{ts}.csv"
    trigger_path = out_dir / f"custom_{tag}_trigger_{ts}.csv"
    ...
    print("\n=== Trigger Counts (Top 80 rows) ===")
    print(trigger.head(80).to_string(index=False))
    print("\nSaved files:")
    print(raw_path)
    print(summary_path)
    print(hold_path)

"""Run a fixed-exit five-year comparison for baseline MACD and pre-cross momentum entries."""
...
ENTRY_VARIANTS = [
    ...
    },
    {
        ...
        "rule_summary": (
            "Buy when MACD histogram rises for 2 bars below zero and price rises "
            "for 2 bars; execute at the next open."
        ),
    },
    {
        "label": "Peak3Bar",
        ...
        "rule_summary": (
            "Require the 3-bar rising window to start at the most negative MACD histogram "
            "bar in the current below-zero segment."
        ),
    },
    {
        "label": "Peak2Bar",
        ...
        "rule_summary": (
            "Require the 2-bar rising window to start at the most negative MACD histogram "
            "bar in the current below-zero segment."
        ),
    },
]
...
def _build_report(summary_df, stage_df, output_dir, topix_return_pct):
    ...
    comparisons = []
    for label in [row["label"] for row in ENTRY_VARIANTS if row["label"] != "Baseline"]:
    ...
    lines = [
        "# MACD Pre-Cross Variant Comparison",
        "",
    ...
    report_path = output_dir / "macd_precross_entry_5y_report.md"
...
def main():
    ...
    output_dir = ROOT / "strategy_evaluation" / f"macd_precross_entry_5y_{ts}"

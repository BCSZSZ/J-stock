...
class StockDataManager:
    ...
    def _atomic_save(self, df: pd.DataFrame, target_path: Path) -> None:
        ...
        with tempfile.NamedTemporaryFile(
            mode="wb", suffix=".parquet", dir=temp_dir, delete=False
        ) as tmp:
    ...
    def compute_features(
        ...
        if not raw_path.exists():
            logger.warning(
                f"[{code}] No raw price data found. Run fetch_and_update_ohlc first."
            )
            return pd.DataFrame()
        ...
        if df.empty or len(df) < 200:
            logger.warning(
                f"[{code}] Insufficient data for indicators (need 200+ rows)"
            )
            return df
        ...
        # KDJ (9, 3, 3) for MACD+KDJ strategy
        stoch_9 = StochasticOscillator(
            high=df["High"], low=df["Low"], close=df["Close"], window=9, smooth_window=3
        )
        df["KDJ_K_9"] = stoch_9.stoch()
        df["KDJ_D_9"] = stoch_9.stoch_signal()
    ...
    def fetch_and_save_financials(self, code: str) -> Optional[pd.DataFrame]:
        ...
        if output_path.exists():
            ...
            if len(merged_df) > len(existing_df):
                logger.info(
                    f"[{code}] Added {len(merged_df) - len(existing_df)} new financial records. Total: {len(merged_df)}"
                )
                self._atomic_save(merged_df, output_path)
            else:
                logger.info(
                    f"[{code}] Financials up-to-date, no new records ({len(existing_df)} records)"
                )
                return existing_df
    ...
    def fetch_and_save_investor_trades(self, code: str) -> Optional[pd.DataFrame]:
        ...
        if output_path.exists():
            ...
            if start_date.date() >= end_date.date():
                logger.info(
                    f"[{code}] Investor trades up-to-date (last: {last_date.date()})"
                )
                return existing_df
        else:
        ...
        if not existing_df.empty:
            ...
            available_cols = [
                col
                for col in ["EnDate", "Section", "InvestorCode"]
                if col in merged_df.columns
            ]
            if available_cols:
                merged_df = merged_df.drop_duplicates(
                    subset=available_cols, keep="last"
                )
            ...
            if len(merged_df) > len(existing_df):
                logger.info(
                    f"[{code}] Added {len(new_df)} new trade records. Total: {len(merged_df)} (was {len(existing_df)})"
                )
            else:
                logger.info(
                    f"[{code}] No new records after dedup. Total: {len(merged_df)}"
                )
        else:
    ...
    def fetch_and_save_metadata(self, code: str) -> None:
        ...
        earnings = self.client.get_earnings_calendar(
            code, today.strftime("%Y-%m-%d"), future.strftime("%Y-%m-%d")
        )
    ...
    def run_full_etl(self, code: str, force_recompute: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            df_features = self.compute_features(code, force_recompute=force_recompute)

...
class StockETLPipeline:
    ...
    def _run_basic_etl(
        ...
        try:
            ...
            # 只有新数据才重算 features
            df_features = self.manager.compute_features(
                code, force_recompute=force_recompute
            )
    ...
    def _run_features_recompute(self, code: str) -> Dict[str, Any]:
        """
        Recompute features only (no network fetch).
        ...
        try:
            df_features = self.manager.compute_features(code, force_recompute=True)
            if df_features.empty:
                result["errors"].append("No price data for features recompute")
                return result
            ...
            logger.error(f"[{code}] Features recompute failed: {e}")
            result["errors"].append(str(e))

...
class MACDEnhancedFundamentalStrategy(BaseEntryStrategy):
    ...
    def generate_entry_signal(self, market_data: MarketData) -> TradingSignal:
        ...
        if entry_gate:
            # 满足进场条件
            reasons.append(
                f"✓ Entry gate passed (RS > {self.rs_threshold} OR Bias > {self.bias_threshold})"
            )
            return TradingSignal(
            ...
            # 金叉但未通过进场门槛
            reasons.append(
                f"✗ Entry gate failed (RS={rs_score:.3f} <= {self.rs_threshold}, Bias={bias_score:.3f} <= {self.bias_threshold})"
            )
            return TradingSignal(
                action=SignalAction.HOLD,
                confidence=confidence,
    ...
    # ===== 维度1：相对强度（RS）连续评分 =====
    def _score_relative_strength_continuous(
        self, df: pd.DataFrame, current_date
    ) -> float:
        """
        ...
        try:
            ...
            try:
                ...
                topix_return_20d = (
                    topix_price_end - topix_price_start
                ) / topix_price_start

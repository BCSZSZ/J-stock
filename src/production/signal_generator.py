...
from ..analysis.signals import TradingSignal, SignalAction, MarketData, Position
...
class SignalGenerator:
    ...
    def __init__(
        ...
        # Load monitor list
        monitor_file = config.get('monitor_list_file', 'data/production_monitor_list.json')
        with open(monitor_file, 'r', encoding='utf-8') as f:
            monitor_data = json.load(f)
            self.monitor_list = monitor_data.get('tickers', [])
    ...
    def evaluate_all_groups(
        ...
        for group_config in strategy_groups:
            ...
            if verbose:
                ...
                sell_count = len([s for s in signals if s.signal_type in ["SELL", "EXIT"]])
    ...
    def _evaluate_group(
        ...
        position_tickers = set(pos.ticker for pos in group.positions if pos.quantity > 0)
    ...
    def _evaluate_exit(
        ...
        try:
            ...
            if trading_signal.action == SignalAction.SELL:
                ...
                holding_days = (pd.Timestamp(current_date) - pd.Timestamp(position.entry_date)).days
    ...
    def _evaluate_entry(
        ...
        try:
            ...
            if trading_signal.action == SignalAction.BUY and score >= buy_threshold:
                ...
                suggested_qty = int(max_position_value / current_price) if current_price > 0 else 0

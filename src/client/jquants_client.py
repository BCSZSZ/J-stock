...
class _GlobalRateLimiter:
    ...
    @classmethod
    def _get_delay_seconds(cls) -> float:
        rpm_raw = os.getenv("JQUANTS_RATE_LIMIT_RPM", "55")
        try:
            rpm = float(rpm_raw)
        except ValueError:
            rpm = 60.0
...
class JQuantsV2Client:
    ...
    def __init__(self, api_key: str):
        ...
        delay = _GlobalRateLimiter._get_delay_seconds()
        logger.info(
            "JQuants rate limiter: %.2f req/min (%.2fs/request)",
            60.0 / delay,
            delay,
        )
    ...
    def get_investor_types(
        self, code: str, from_date: Optional[str] = None, to_date: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_topix_bars(self, from_date: str, to_date: str) -> List[Dict[str, Any]]:
        """

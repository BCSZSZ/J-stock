...
- Configurable worker count for optimal IO throughput
...
class UniverseSelector:
    ...
    def run_selection(
        ...
        if apply_filters:
            ...
            logger.info(
                f"Filters disabled. Using all {len(df_filtered)} stocks for scoring"
            )
    ...
    def fetch_universe_data(
        ...
        if ticker_list:
            ...
            if not listed_stocks:
                ...
                logger.warning(
                    "TIP: For testing, you can provide ticker_list parameter"
                )
                return pd.DataFrame()
        ...
        # Extract features for each stock (PARALLEL)
        total = len(df_listed)
        logger.info(
            f"🚀 Starting parallel feature extraction ({self.workers} workers)..."
        )
        ...
        if self.workers > 1:
            ...
            with ThreadPoolExecutor(max_workers=self.workers) as executor:
                ...
                for idx, row in df_listed.iterrows():
                    ...
                    future = executor.submit(
                        self._extract_stock_features, code, name, no_fetch
                    )
                ...
                for future in as_completed(future_to_stock):
                    ...
                    try:
                        ...
                        if features:
                            ...
                            logger.info(f"[{completed}/{total}] ✓ {code} - {name}")
                        else:
                            logger.warning(
                                f"[{completed}/{total}] ✗ {code} - Insufficient data"
                            )
                    except Exception as e:
                        logger.error(f"[{completed}/{total}] ✗ {code} - Error: {e}")
        else:
            ...
            for idx, row in df_listed.iterrows():
                ...
                logger.info(f"[{completed}/{total}] Processing {code} - {name}")

                try:
                    features = self._extract_stock_features(
                        code, name, no_fetch=no_fetch
                    )
                    if features:
                        results.append(features)
                except Exception as e:
        ...
        logger.info(
            f"✅ Successfully extracted features for {len(df_universe)}/{total} stocks"
        )
    ...
    def _extract_stock_features(
        self, code: str, name: str, no_fetch: bool = False
    ) -> Optional[Dict]:
        """
        ...
        if no_fetch:
            ...
            if df_features.empty or len(df_features) < self.MIN_HISTORY_DAYS:
                logger.warning(
                    f"[{code}] No local data available (use without --no-fetch first)"
                )
                return None
        else:
            ...
            if df_prices.empty or len(df_prices) < self.MIN_HISTORY_DAYS:
                logger.warning(
                    f"[{code}] Insufficient price data ({len(df_prices)} days)"
                )
                return None
        ...
        if pd.isna(trend_strength):
            trend_strength = (
                ...
                if (ema_200 > 0 and not np.isnan(ema_200))
                else np.nan
            )
        ...
        if pd.isna(momentum_20d):
            if len(df_features) >= 21:
                ...
                momentum_20d = (
                    (close - price_20d_ago) / price_20d_ago
                    if price_20d_ago > 0
                    else np.nan
                )
            else:
    ...
    def apply_hard_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"  - Price filter (>{self.MIN_PRICE} JPY): "
        ...
        logger.info(
            f"  - Liquidity filter (>{self.MIN_LIQUIDITY / 1e6:.0f}M JPY): "
    ...
    def normalize_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_norm["Rank_Momentum"] = df_norm["Momentum_20d"].rank(
            pct=True, ascending=True
        )

        # Rank_VolSurge: Higher volume surge = Higher Score
        df_norm["Rank_VolSurge"] = df_norm["Volume_Surge"].rank(
            pct=True, ascending=True
        )
    ...
    def save_selection_results(
        self, df_top: pd.DataFrame, format: str = "both"
    ...
    def save_scores_txt(
        self, df_scored: pd.DataFrame, df_top: pd.DataFrame, top_n: int = 50
    ) -> Path:
        """

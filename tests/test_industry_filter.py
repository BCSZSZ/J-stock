...
def _reference_file(tmp_path: Path) -> Path:
    path = tmp_path / "jpx_final_list.csv"
    path.write_text(
        "\n".join(
            [
...
def test_get_industry_name_loads_jpx_reference_file(tmp_path: Path) -> None:
...
def test_industry_filter_keeps_first_ranked_buy_per_day(tmp_path: Path) -> None:
    reference_file = _reference_file(tmp_path)
...
def test_industry_filter_blocks_when_total_position_cap_is_reached(
    tmp_path: Path,
) -> None:
    reference_file = _reference_file(tmp_path)
    config = IndustryFilterConfig(
...
def test_industry_filter_shadow_mode_marks_without_filtering(tmp_path: Path) -> None:
    reference_file = _reference_file(tmp_path)
    config = IndustryFilterConfig(
        mode="shadow",
        max_buy_per_industry_per_day=1,

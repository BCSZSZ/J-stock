# Step-1 Exit Screening (A): representative entries x coarse exit families
...
try {
    Write-Host "=== Step-1 Exit Screening (A) ===" -ForegroundColor Cyan

    $entryStrategies = @(
        Get-Content strategy_evaluation/exit_a_entry_strategies.txt |
        Where-Object { $_ -and $_.Trim().Length -gt 0 }
    )

    $exitStrategies = @(
        Get-Content strategy_evaluation/exit_a_exit_strategies.txt |
        Where-Object { $_ -and $_.Trim().Length -gt 0 }
    )

    if ($entryStrategies.Count -eq 0) {
        throw "No entry strategies in strategy_evaluation/exit_a_entry_strategies.txt"
    }
    if ($exitStrategies.Count -eq 0) {
        throw "No exit strategies in strategy_evaluation/exit_a_exit_strategies.txt"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Step-1 exit screening failed"
    }
